...
const RecruiterDashboard = () => {
  ...
  const jobData = {
    ...
    postedBy: recruiterEmail, // 👈 Save recruiter email
  };
  ...
  try {
    ...
    setActivePage("dashboard"); // go back to dashboard
  } catch (error) {
...
const MyJobs = () => {
  ...
  return (
    <div>
      <h2 style={styles.heading}>My Jobs</h2>
      {jobs.length === 0 ? (
        ...
        <ul>
          {jobs.map((job) => (
            <li key={job.id}>
              <strong>{job.title}</strong> — {job.location} ({job.jobType})
            </li>
          ))}
        </ul>
      )}
  ...
  {activePage === 'myJobs' && <MyJobs />}
  {activePage === 'candidates' && <h2>Candidates Page (Coming Soon)</h2>}
  {activePage === 'shortlisted' && <h2>Shortlisted Candidates (Coming Soon)</h2>}
